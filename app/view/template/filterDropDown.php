...
<div class="relative filter-dropdown-container" 
    ...
    <button onclick="toggleDropdown('<?= $filter_id ?>_menu')" type="button" 
        ...
        <img src="/icon/arrowDown.svg" alt="dropDown" class="w-4 h-4 ml-auto">
    </button>

    <div id="<?= $filter_id ?>_menu" 
         class="hidden absolute z-20 mt-2 w-56 bg-white border border-gray-200 rounded-lg shadow-xl origin-top-left max-h-60 overflow-y-auto filter-dropdown-menu">
        <ul class="py-1">
            <?php foreach ($options as $option_label => $option_value): ?>
                ...
                ?>
                <li class="px-4 py-2 hover:bg-gray-50 transition">
                    <label for="<?= $clean_id ?>" 
                           class="flex items-center cursor-pointer space-x-2 text-sm text-gray-700">
                        ...
                        <input type="checkbox"
                               ...
                               class="h-4 w-4 text-[#1E68FB] border-gray-300 rounded focus:ring-[#1E68FB]">
